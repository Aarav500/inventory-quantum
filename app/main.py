...
# /health must be registered before app.mount() so Starlette's route-match
# loop finds it before the StaticFiles Mount, which returns 405 for HEAD on
# paths it does not own and would otherwise short-circuit the search.
...
# Mount static files — after /health and /debug routes
